...
/***
* Returns a formatted string of current date..time..
* see "man strftime" for different formats.
***/
void main() {
   printf("\n%s\n\n", date("%B %F %r"));
}
...
/*
%a The abbreviated name of the day of the week according to the current  lo‐
cale.  (Calculated from tm_wday.)

%A The  full  name  of  the day of the week according to the current locale.
(Calculated from tm_wday.)

%b The abbreviated month name according to the current locale.   (Calculated
from tm_mon.)
...
%d The  day  of the month as a decimal number (range 01 to 31).  (Calculated
from tm_mday.)
...
replaced by a space. (SU) (Calculated from tm_mday.)

%E Modifier: use alternative format, see below. (SU)

%F Equivalent to %Y-%m-%d (the ISO 8601 date format). (C99)

%G The ISO 8601 week-based year (see NOTES) with century as a  decimal  num‐
ber.   The  4-digit  year  corresponding to the ISO week number (see %V).
...
%j The  day of the year as a decimal number (range 001 to 366).  (Calculated
from tm_yday.)

%k The hour (24-hour clock) as a decimal number (range 0 to 23); single dig‐
its  are preceded by a blank.  (See also %H.)  (Calculated from tm_hour.)
(TZ)

%l The hour (12-hour clock) as a decimal number (range 1 to 12); single dig‐
its  are preceded by a blank.  (See also %I.)  (Calculated from tm_hour.)
(TZ)
...
%p Either "AM" or "PM" according to the given time value, or the correspond‐
ing strings for the current locale.  Noon is treated as "PM" and midnight
as "AM".  (Calculated from tm_hour.)

%P Like %p but in lowercase: "am" or "pm" or a corresponding string for  the
current locale.  (Calculated from tm_hour.)  (GNU)

%r The  time  in a.m. or p.m. notation.  In the POSIX locale this is equiva‐
lent to %I:%M:%S %p.  (SU)
...
and %W.  (Calculated from tm_yday and tm_wday.)
...
4 days in the new year.  See also %U and %W.  (Calculated  from  tm_year,
tm_yday, and tm_wday.)  (SU)
...
%y The year as a decimal number without a century (range 00 to 99).  (Calcu‐
lated from tm_year)

%Y The  year  as  a  decimal number including the century.  (Calculated from
tm_year)
